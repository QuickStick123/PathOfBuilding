...
writeMods("../Data/Uniques/Special/Uniques.lua", function(mod)
	return not (mod.Family[1].Id == "AuraBonus" or mod.Family[1].Id == "ArbalestBonus") and not mod.Family[1].Id:match("^EnchantmentHeist") and not mod.Id:match("^Synthesis") and not mod.Id:match("Royale") and mod.GenerationType == 3
...
local out = io.open("../Data/Uniques/Special/ModTextMap.lua", "w")
local modTextMap = {}
out:write('-- This file is automatically generated, do not edit!\n')
out:write('-- Item data (c) Grinding Gear Games\n\nreturn {\n')
for modName, mod in pairs(LoadModule("../Data/Uniques/Special/Uniques.lua")) do
	local combinedMod = table.concat(mod, "|")
	local rangedRemovedLine, ranges = extractRanges(combinedMod)
	if modTextMap[rangedRemovedLine] and modTextMap[rangedRemovedLine][combinedMod] then
		table.insert(modTextMap[rangedRemovedLine][combinedMod].modNames, modName)
	elseif modTextMap[rangedRemovedLine] then
		modTextMap[rangedRemovedLine][combinedMod] = { modNames = { modName }, ranges = ranges }
	else
		modTextMap[rangedRemovedLine] = { [combinedMod] = { modNames = { modName }, ranges = ranges } }
	end
end
for rangedRemovedLines, modGroup in pairs(modTextMap) do
	out:write('\t["' .. rangedRemovedLines .. '"] = { ')
	for exactMod, mod in pairs(modGroup) do
		out:write('\n\t\t["' .. exactMod .. '"] = { ')
		out:write('modNames = { "'.. table.concat(mod.modNames, '", "') .. '" }')
		if #mod.ranges > 0 then
			out:write(', ranges = { ')
			for i, range in ipairs(mod.ranges) do
				out:write('{ ' .. range[1] .. ', ' .. range[2] .. ' }')
				if i < #mod.ranges then out:write(", ") end
			end
			out:write(' }')
		end
		out:write(' },')
